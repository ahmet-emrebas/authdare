import { DatabaseManager, DATABASE_MANAGER_TOKEN, Groups, Login, UserEntity, UserPermission } from '@authdare/models';
...
export class AuthService {
  ...
  async join(user: UserEntity) {
    ...
    const userInstance = await new UserEntity(user).validateAndTransformToClassInstance([Groups.SIGNUP]);
    ...
    try {
      ...
      const plainUser = classToPlain(createdUser) as UserEntity;
  ...
  async signup(user: UserEntity) {
    ...
    userInstance.permissions = undefined;
