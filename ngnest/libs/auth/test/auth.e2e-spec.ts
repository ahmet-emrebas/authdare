...
describe('Auth Module', () => {
    ...
    describe('Sign up', () => {
        it('POST /auth/signup', async () => {
            const res = await request.post(AuthRoutes.SIGNUP).send(validSignupForm);
            assert.equal(res.status, 201);
            assert.deepEqual(res.body, signupResponseBody);
        });

        it('POST /auth/login', async () => {
            const res = await request.post(AuthRoutes.LOGIN).send(validLoginForm);
        ...
        it('GET /auth/profile', async () => {
            const res = await request.get(AuthRoutes.PROFILE).set('Cookie', cookies).send();
        ...
        it(`PATCH /auth/update-profile`, async () => {
            ...
            const res = await request.patch(AuthRoutes.UPDATE_PROFILE).set('Cookie', cookies).send({ password: updatedPassword });
