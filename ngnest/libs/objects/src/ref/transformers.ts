 ...
 * @returns
 */
export const ToArray = (delimeter = ',') =>
    Transform(({ value }) => isNotEmpty(value) && split(value, delimeter), {
        toClassOnly: true,
    });
...
export const ToObject = () =>
    Transform(
        ({ value }) => {
            return (
                isNotEmpty(value) &&
                fromPairs(split(value, ',')?.map((e) => e?.split(':')))
            );
        },
        { toClassOnly: true },
    );
 ...
 * @returns
 */
export const ToBoolean = () => Transform(({ value }) => (value == 'false' ? false : !!value), {
    toClassOnly: true,
});