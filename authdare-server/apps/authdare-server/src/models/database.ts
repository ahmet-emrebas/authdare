...
export interface DatabaseManager<P> {
    orgConnection(orgname: string, synchronize?: boolean, dropSchema?: boolean): Promise<Connection>;
    getResouceNames(): string[];
    getEntities(): any[];
    getEntityByResourceName(resourceName: string);
    getRepositoryByOrgname<E = any>(params: DatabaseParameters): Promise<Repository<E>>;
    getUserRepositoryByOrgname<E = any>(orgname: string): Promise<Repository<E>>;
    adminPermissions(): P[];
}

@Injectable()
export class SQLiteDatabasaManager<PermissinType> implements DatabaseManager<PermissinType> {
     ...
     * @returns 
     */
    async orgConnection(orgname: string, synchronize: boolean = false, dropSchema: boolean = false): Promise<Connection> {
        let con: Connection;
        try {
    ...
    async getRepositoryByOrgname<E>({ orgname, resource }: DatabaseParameters) {
        let con = await this.orgConnection(orgname);
     ...
     * @returns 
     */
    async getUserRepositoryByOrgname<E = any>(orgname: string): Promise<Repository<E>> {
